...
return [
    ...
    'form' => [
        'section' => [
            ...
            'dkim' => 'DKIM signing (optional)',
            'dkim_description' => 'NOT needed when using Mailgun — Mailgun signs with its own key on their server (you only add the public key TXT in DNS). Enter the private key ONLY if: (a) you use an SMTP relay that does NOT DKIM-sign (e.g. your own Postfix without opendkim), (b) you want an additional second signature (dual signing). The key must match the `selector._domainkey.domain` TXT record in DNS.',
        ...
        'helper' => [
            ...
            'dkim_domain' => 'Domain to sign for, e.g. `hovera.pl`. Must match the `d=` tag in the resulting DKIM-Signature header.',
            'dkim_selector' => 'DKIM selector, e.g. `mailo` or `default`. Must have a matching TXT record in DNS: `{selector}._domainkey.{domain}` with the public key.',
            'dkim_private_key' => 'Full PEM private key with `-----BEGIN RSA PRIVATE KEY-----` / `-----END RSA PRIVATE KEY-----` headers (or PKCS#8 `BEGIN PRIVATE KEY`). Leave empty to keep the previous value.',
        ...
        'status' => [
            ...
            'dkim_active' => '✓ DKIM signing active — every outgoing email gets a DKIM-Signature header',
            'dkim_inactive' => '⚠ DKIM signing disabled (Mailgun signs on its own, so this is fine when using Mailgun)',
        ],
    ],
