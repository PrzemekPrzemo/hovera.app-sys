...
return [
    ...
    'terms' => [
        ...
        'section_2_body' => 'The Service includes: lesson calendar, client and horse records, billing (invoices in PLN and foreign currencies with automatic conversion at NBP exchange rates pursuant to art. 31a sec. 1 of the Polish VAT Act, online payments via P24/PayU/Stripe, KSeF integration pursuant to art. 106e sec. 11 of the Polish VAT Act), public stable micro-site, online booking form, customer communications (email, SMS, WhatsApp deep links), financial reports, transport fleet management (including individual driver accounts with views of their own routes and PLW document status), transport inquiry marketplace, customer VAT ID validation (Polish IDs via GUS/REGON, CEIDG, KRS; EU IDs via the European Commission VIES system), and related features. The full feature list per Plan is available at /pricing. For the transport module (marketplace of inquiries from stables to transport companies), the Provider acts EXCLUSIVELY as a technology intermediary — it does not perform transports, does not own vehicles, does not employ drivers, and is not a party to the transport contract. Detailed marketplace rules are governed by the separate <a href="/regulamin-marketplace">Transport Marketplace Terms</a>, which form an integral part of these Terms for Customers using the transport module. The "Share offer via WhatsApp" feature generates a `wa.me/{phone}?text=...` deep link that opens the Customer\'s WhatsApp application — Hovera does NOT send messages through the WhatsApp Business API, does NOT transfer data to Meta Platforms Inc., and the message is dispatched solely from the User\'s device.',
    ...
    'privacy' => [
        ...
        'section_2_body' => 'During registration: full name, email, stable name, stable URL. During use of the service: IP address, session and cookie identifiers, device technical data (user-agent), activity logs (login time, panel actions). Data entered by the Customer about its clients (riders, horses) is processed as a processor — under the DPA terms (/dpa). In the transport module, carrier companies may create individual driver accounts (first name, last name, email, phone, driver licence number with expiry date, optionally ADR / animal transport competency certificates with expiry dates) — drivers log in and see only their own routes and the status of their own documents. Entrusting this data to Hovera takes place under the agreement between the carrier company (Controller) and Hovera (Processor) — DPA applies accordingly.',
        ...
        'section_5_body' => 'Data may be entrusted to: Hetzner Online GmbH (hosting, DE/EU), OVH SAS (backups, FR/EU), Stripe Payments Europe Ltd. (card payments, IE/EU), PayU SA and Przelewy24 / DialCom24 sp. z o.o. (Polish online payments, PL), SMSAPI sp. z o.o. (SMS, PL), transactional email provider (e.g. Postmark/Mailgun — EU/US under SCC), Polish Ministry of Finance (KSeF — structured invoice submission, PL), National Bank of Poland (public NBP currency rates API — no personal data transferred, PL), Statistics Poland (BIR/REGON API — Polish VAT ID validation, PL), CEIDG (sole-trader registry, PL), European Commission (public VIES API — EU VAT ID validation, BE/EU). Current list available on request at office@hovera.app.',
    ...
    'marketplace' => [
        ...
        'section_6_body' => 'Upon clicking "Accept quote" on /transport/quote/{slug}/{token}, the Customer concludes a direct transport contract with the Transporter named in the quote, on the terms (price, date, route, additional conditions) set out in the quote text and the attached PDF. On the acceptance page the Customer indicates the invoice recipient type: (a) individual — name only is sufficient, no VAT ID required (under Polish tax law, an invoice for an individual not running a business does not require a VAT ID); (b) company — VAT ID, name and address required; for Polish VAT IDs Hovera auto-fills data from GUS/CEIDG/KRS, and for EU VAT IDs it validates the number in the European Commission VIES system. Hovera only technically relays the acceptance to the Transporter and notifies other Transporters that the inquiry is closed (auto-reject). Hovera IS NOT a party to this contract, DOES NOT GUARANTEE its performance, and IS NOT LIABLE for its execution. Customer and Transporter independently settle additional matters (invoice, deposit, cancellation, meeting place and time) — directly via the contact channel given in the quote or on the Transporter\'s profile.',
