...
| `file_put_contents storage/framework/views/...Permission denied` lub `storage/logs/laravel-...log Permission denied` | PHP-FPM user (Plesk: vhost user, VPS: `www-data`) nie ma write na `storage/`. **`./update.sh` MUSI być uruchomione jako root lub przez `sudo`** (chown wymaga uprawnień admin'a). Bez sudo chown cicho failuje. Manual fix: `sudo chown -R $(stat -c '%U:%G' index.php) storage bootstrap/cache && sudo chmod -R 775 storage bootstrap/cache && sudo find storage -type d -exec chmod 2775 {} \;`. |
