...
| `file_put_contents storage/framework/views/...Permission denied` lub `storage/logs/laravel-...log Permission denied` | PHP-FPM user (Plesk: vhost user, VPS: `www-data`) nie ma write na `storage/`. `deploy.sh` od PR-A robi auto-chown + sticky bit, ale jeśli zostałeś bez deploy'a manualnie: `sudo chown -R www-data:www-data storage bootstrap/cache && sudo chmod -R 775 storage bootstrap/cache && sudo find storage -type d -exec chmod 2775 {} \;`. Na Plesku podstaw `www-data` user'em vhost'a (np. `psaserv` lub konkretną nazwę z `id`). |
